...
describe('promptci doctor CLI command', () => {
  ...
  it('passes cleanly when Node version, config, and gitignore are configured', async () => {
    ...
    // doctor reports the RUNNING CLI version (from its own package.json), not
    // whatever a globally-linked `promptci` on PATH resolves to.
  ...
  it('doctor performs no network requests', async () => {
    Object.defineProperty(process, 'versions', {
      value: { node: '22.0.0' },
      configurable: true,
    });
