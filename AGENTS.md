...
`warning`, compared to the committed `.promptci/baseline.json` (see
[.github/workflows/ci.yml](.github/workflows/ci.yml)). The self-scan is the highest-signal
demonstration that the tool works, so keep it green:

- Never commit .promptci output — the generated Markdown and JSON reports are ignored by
  [.gitignore](.gitignore). Only the shared `.promptci/baseline.json` and `.promptci/config.json`
  are committed.
- If a change legitimately alters instruction health, refresh the baseline in the same PR with
  `pnpm selfscan:update` and explain why in the PR description — do not silently widen it.
